    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-body">
                        <form role="form" action="{{ route('industries.update', ['industry' => $industry->id]) }}" method="post">
                            @csrf
                            @method("PUT")
                            <div class="form-group mb-3 col-6 mg-col-12 ">
                                <label for="group_industries_id" class="form-label">Группа отраслей</label>
                                <select class="form-select @error('group_industries_id') is-invalid @enderror" id="group_industries_id" name="group_industries_id">
                                    @foreach($groups as $group)
                                        <option value="{{ $group->id }}" {{ $industry->group_industries_id == $group->id ? 'selected' : '' }}>{{ $group->title }}</option>
                                    @endforeach
                                </select>
