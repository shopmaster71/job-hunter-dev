@extends('layouts.app')
@section('title',  'Изменение вакансии '.$vacancy->position)
@section('description', 'Изменение вакансии '.$vacancy->position)
                ...
                <div class="flex flex-col-reverse items-start lg:flex-row justify-start gap-3 lg:gap-6 w-full">
                    <div class="w-full lg:w-2/3 bg-white  rounded-lg shadow-lg py-5 px-5 lg:py-10 lg:px-10">
                        ...
                        <div class="mt-2 px-0 md:px-4">
                            <form action="{{ route('agency.vacancy.update', ['vacancy' => $vacancy->id]) }}" method="post">
                                @csrf
                                @method('PUT')
                                                    ...
                                                    @foreach($industries as $industryGroup)
                                                        <optgroup label="{{ $industryGroup->title }}">
                                                            @foreach($industryGroup->industries as $industry)
                                                                <option value="{{ $industry->id }}" {{ $vacancy->industry_id == $industry->id ? "selected":'' }}>{{ $industry->title }}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="w-full profile-fields relative border-b border-[#cccccc] ">
                                                <select class="select block w-full new_select" name="employment_type_id">
                                                    @foreach($employmentTypes as $employmentType)
                                                        <option value="{{ $employmentType->id }}" {{ $vacancy->employment_type_id == $employmentType->id ? 'selected' : ''  }}>{{ $employmentType->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="w-full profile-fields relative border-b border-[#cccccc] ">
                                                <select class="select block w-full new_select" name="expertise_id">
                                                    @foreach($expertises as $expertise)
                                                        <option value="{{ $expertise->id }}" {{ $vacancy->expertise_id == $expertise->id ? 'selected' : ''  }}>{{ $expertise->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="w-full custom_select profile-fields relative border-b border-[#cccccc] ">
                                                <select class="select_default block w-full py-1 text-blue-primary bg-transparent appearance-none focus:outline-none focus:ring-0 peer" name="specialization_id" >
                                                    @foreach($specializations as $specialization)
                                                        <option value="{{ $specialization->id }}" {{ $vacancy->specialization_id == $specialization->id ? 'selected' : ''  }}>{{ $specialization->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="w-full profile-fields relative border-b border-[#cccccc] ">
                                                <select class="select block w-full new_select" name="schedule_id">
                                                    @foreach($schedules as $schedule)
                                                        <option value="{{ $schedule->id }}" {{ $vacancy->schedule_id == $schedule->id ? 'selected' : ''  }}>{{ $schedule->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            ...
                                            <div class="w-full profile-fields relative border-b border-[#cccccc] ">
                                                <select class="select block w-full new_select" name="format_id">
                                                    @foreach($formats as $format)
                                                        <option value="{{ $format->id }}" {{ $vacancy->format_id == $format->id ? 'selected' : ''  }}>{{ $format->title }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                ...
                                <div class="flex flex-col gap-0 mt-2">
                                    ...
                                    <div class="flex mt-3 items-start">
                                        <label class="flex items-center cursor-pointer relative">
                                            <input type="checkbox" name="additional[]" class="peer h-4 w-4 cursor-pointer transition-all appearance-none rounded hover:shadow-md border border-[#cccccc] checked:bg-green-primary checked:border-green-primary" id="disabilities" value="disabilities" {{ in_array('disabilities', $vacancy->additional) ? 'checked' : '' }} />
                                            <span class="absolute text-white opacity-0 peer-checked:opacity-100 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" stroke-width="1">
                                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                                </svg>
                                    ...
                                    <div class="flex mt-1.5 items-start">
                                        <label class="flex items-center cursor-pointer relative">
                                            <input type="checkbox" name="additional[]" class="peer h-4 w-4 cursor-pointer transition-all appearance-none rounded hover:shadow-md border border-[#cccccc] checked:bg-green-primary checked:border-green-primary" id="moving" value="moving" {{ in_array('moving', $vacancy->additional) ? 'checked' : '' }} />
                                            <span class="absolute text-white opacity-0 peer-checked:opacity-100 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" stroke-width="1">
                                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                                </svg>
                                    ...
                                    <div class="flex mt-1.5 items-start">
                                        <label class="flex items-center cursor-pointer relative">
                                            <input type="checkbox" name="additional[]" class="peer h-4 w-4 cursor-pointer transition-all appearance-none rounded hover:shadow-md border border-[#cccccc] checked:bg-green-primary checked:border-green-primary" id="students" value="students" {{ in_array('students', $vacancy->additional) ? 'checked' : '' }} />
                                            <span class="absolute text-white opacity-0 peer-checked:opacity-100 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" stroke-width="1">
                                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                                </svg>
                                    ...
                                    <div class="flex mt-1.5 items-start">
                                        <label class="flex items-center cursor-pointer relative">
                                            <input type="checkbox" name="additional[]" class="peer h-4 w-4 cursor-pointer transition-all appearance-none rounded hover:shadow-md border border-[#cccccc] checked:bg-green-primary checked:border-green-primary" id="pensioners" value="pensioners" {{ in_array('pensioners', $vacancy->additional) ? 'checked' : '' }} />
                                            <span class="absolute text-white opacity-0 peer-checked:opacity-100 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" stroke-width="1">
                                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                                </svg>
                                    ...
                                    <div class="flex mt-1.5 items-start">
                                        <label class="flex items-center cursor-pointer relative">
                                            <input type="checkbox" name="additional[]" class="peer h-4 w-4 cursor-pointer transition-all appearance-none rounded hover:shadow-md border border-[#cccccc] checked:bg-green-primary checked:border-green-primary" id="phone" value="phone" {{ in_array('phone', $vacancy->additional) ? 'checked' : '' }} />
                                            <span class="absolute text-white opacity-0 peer-checked:opacity-100 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" stroke="currentColor" stroke-width="1">
                                                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                                </svg>
