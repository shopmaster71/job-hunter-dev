...
@section('description', 'Новая вакансия')
                ...
                <div class="flex flex-col-reverse items-start lg:flex-row justify-start gap-3 lg:gap-6 w-full">
                    <div class="w-full lg:w-2/3 bg-white  rounded-lg shadow-lg py-5 px-5 lg:py-10 lg:px-10">
                        ...
                        <div class="mt-2 px-0 md:px-4">
                            <form action="{{ route('agency.vacancy.store') }}" method="post">
                                @csrf
                                    ...
                                    <div class="w-full lg:w-1/2 flex flex-col gap-4 mt-2">
                                        ...
                                        <div class="flex flex-col gap-0">
                                            ...
                                            <div class="w-full profile-fields custom_select relative border-b border-[#cccccc] ">
                                                <select class="select_default block w-full py-1 text-blue-primary bg-transparent appearance-none focus:outline-none focus:ring-0 peer" name="industry_id" id="industry_id">
                                    ...
                                    <div class="w-full lg:w-1/2 flex flex-col gap-4 mt-2">
                                        ...
                                        <div class="flex flex-col gap-0">
                                            ...
                                            <div class="w-full custom_select profile-fields relative border-b border-[#cccccc] ">
                                                <select class="select_default block w-full py-1 text-blue-primary bg-transparent appearance-none focus:outline-none focus:ring-0 peer" name="specialization_id" id="specialization_id">
                                ...
                                <div class="flex flex-col gap-0 mt-2">
                                    ...
                                    appearance-none focus:outline-none focus:ring-0 peer" placeholder="Адрес" value="{{ old('address') }}" />
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const specializationsData = @json($specializationsByIndustry ?? []);
            const oldSpecializationId = "{{ old('specialization_id') }}";
            ...
            function updateSpecializations() {
                ...
                if (selectedIndustryId && specializationsData[selectedIndustryId]) {
                    specializationsData[selectedIndustryId].forEach(spec => {
                        const option = document.createElement('option');
                        option.value = spec.id;
                        option.textContent = spec.title;
                        if (spec.id == oldSpecializationId) {
                ...
                if (oldSpecializationId && !specializationSelect.querySelector(`option[value="${oldSpecializationId}"]`)) {
                    console.warn('Специализация не найдена в выбранной отрасли');
                }
            }

            industrySelect.addEventListener('change', updateSpecializations);
