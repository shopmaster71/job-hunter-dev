...
<div class="h-full w-full lg:w-1/2 flex justify-center lg:justify-start items-center bg-white py-3 lg:py-0 pl-3 lg:pl-30 pr-3 lg:pr-5">
    <div class="w-full max-w-[408px]">
        <h2 class="font-semibold text-xl md:text-2xl lg:text-3xl">Авторизация</h2>
        <p class="mt-1 lg:mt-3"><a href="{{ route('register') }}" class="underline">Ещё не зарегистрированы?</a> | <a href="{{ route('forgot') }}" class="underline">Забыли пароль?</a></p>
        <div class="mt-3 lg:mt-13">
            <form action="{{ route('login') }}" method="post" id="form">
                ...
                <div class="relative my-3 lg:my-6">
                    <input type="password" id="password" name="password"
                           class="block w-full px-2 py-2 text-blue-primary bg-transparent border-0 border-b-2 border-[#cccccc]
                    appearance-none focus:outline-none focus:ring-0 peer" placeholder=" " required />
                    <label for="password" class="absolute text-lg text-blue-primary duration-300 transform -translate-y-8 scale-75
                    top-3.5 z-10  left-2 peer-focus:start-0 peer-focus:text-[#cccccc] peer-placeholder-shown:scale-100
                    peer-placeholder-shown:-translate-y-1.5 peer-focus:scale-75 peer-focus:-translate-y-8">Пароль <sup class="text-red-500">*</sup>
                    </label>
                </div>
                <div class="flex items-start mt-3 lg:mt-6">
                    <label class="flex items-center cursor-pointer relative">
                        <input type="checkbox" class="peer h-4 w-4 cursor-pointer transition-all appearance-none rounded hover:shadow-md border border-[#cccccc] checked:bg-green-primary checked:border-green-primary" id="remember" name="remember" />
                    ...
                    <label class="cursor-pointer ml-2 text-[#cccccc] text-sm/4" for="remember">Запомнить меня</label>
                </div>
                ...
                <div class="w-full lg:w-5/8 mt-5 lg:mt-8">
                    <button class="btn-utp-green block w-full grow py-2.5 px-2 text-white text-lg bg-green-primary border border-solid border-green-primary rounded-3xl hover:bg-green-primary-hover hover:border-green-primary-hover cursor-pointer transition duration-150 ease-in-out outline-none" type="submit">Войти</button>
                </div>
            </form>
        </div>
    </div>
