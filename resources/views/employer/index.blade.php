            ...
            <div class="container mx-auto">
                ...
                <div class="w-full flex flex-col gap-3 mt-3 lg:gap-6 lg:mt-6">
                    <div class=" bg-white rounded-lg shadow-lg py-5 px-5 lg:py-10 lg:px-10">
                        <h2 class="font-semibold text-lg md:text-2xl/6">Информация о компании</h2>
                        @if(!$employer)
                                ...
                                <div class="mt-2 lg:mt-4 flex flex-col lg:flex-row gap-3 lg:gap-10">
                                    <div class="w-full lg:w-1/2 flex flex-col gap-4 mt-2">
                                        <div class="flex flex-col md:flex-row gap-1 md:gap-2">
                                            ...
                                            appearance-none focus:outline-none focus:ring-0 peer" placeholder="Электросбыт" value="{{ old('title') }}" />
                                                @if ($errors->has('title'))
                                        ...
                                        <div class="flex flex-col md:flex-row gap-1 md:gap-2">
                                            ...
                                            appearance-none focus:outline-none focus:ring-0 peer" placeholder="Адрес компании" value="{{ old('address') }}"  />
                                    ...
                                    <div class="w-full lg:w-1/2 flex flex-col gap-4 mt-2">
                                        <div class="flex flex-col md:flex-row gap-1 md:gap-2">
                                            ...
                                            appearance-none focus:outline-none focus:ring-0 peer" name="about" placeholder="Расскажите о Вашей компании">{{ old('about') }}</textarea>
                                                @if ($errors->has('about'))
                                            ...
                                            <div class="w-full md:w-5/8 ">
                                                <div class="inline-flex flex-col items-start overflow-hidden">
                                                    <label id="file-label" class="inline-flex items-center pr-2 font-medium text-lg text-[#8F8F8F] focus:outline-none cursor-pointer ">
                                                        ...
                                                        Прикрепите фото
                                                        <input type="file" id="file-input" name="gallery[]" class="hidden" accept="image/*" multiple value="{{ old('gallery') }}" />
                                                    </label>
                        ...
                        @else
                            ...
                            <ul class="grid grid-cols-1 gap-4 mt-6 text-base md:text-lg">
                                <li class="bg-gray-50 py-2 px-3"><b>О компании:</b> {{ $employer->about }}</li>
                            </ul>
                            @if($employer->gallery)
                            ...
                            @endif
                            <div class="mt-6">
                                <a href="{{ route('employer.edit', ['employer' => $employer->id]) }}" class="btn-utp-green inline-block  grow py-2.5 px-14 text-white text-base md:text-lg bg-green-primary border border-solid border-green-primary rounded-3xl hover:bg-green-primary-hover hover:border-green-primary-hover cursor-pointer transition duration-150 ease-in-out outline-none">Изменить</a>
                            </div>
                        @endif
                    </div>
                    <div class=" bg-white rounded-lg shadow-lg py-5 px-5 lg:py-10 lg:px-10">
                        ...
                        @if(!$contacts)
                        <form action="{{ route('employer.contact.create') }}" method="post">
                            @csrf
                        ...
                        @else
                            <ul class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 mt-6 text-base md:text-lg">
                                <li class="bg-gray-50 py-2 px-3"><b>Телефон:</b> {{ $contacts->phone }}</li>
                                <li class="bg-gray-50 py-2 px-3"><b>E-mail:</b> {{ $contacts->email }}</li>
                            </ul>
                            <div class="mt-6">
                                <a href="{{ route('employer.contact.edit', ['employer' => $employer->id]) }}" class="btn-utp-green inline-block  grow py-2.5 px-14 text-white text-base md:text-lg bg-green-primary border border-solid border-green-primary rounded-3xl hover:bg-green-primary-hover hover:border-green-primary-hover cursor-pointer transition duration-150 ease-in-out outline-none">Изменить</a>
                            </div>
                        @endif
                    </div>
